...
const {cloneNode, appendChild, console} = globalThis.Node.prototype;
	...
	const _setNodeValues = (node,i)  => {
		for (let x=0; x<len;x++) {
			...
			if (curNode) {
				...
				log('Field:' + target.dataSlots[x][0] + ' does not exist')
			}
		}	
	}
	const insertRow = appendChild.bind(target)
...
const appendRows = (target, dataSet, start=0) => {
...
export  {createTemplate, appendRows, render, Config , version, prefetchTemplate}
